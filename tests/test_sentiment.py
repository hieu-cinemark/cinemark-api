...
async def test_short_message_skips_backends() -> None:
    with (
        ...
        patch("app.kira.sentiment._classify_kira", AsyncMock()) as mock_kira,
    ):
        result = await classify_sentiment("ok")

    assert result is None
    mock_phobert.assert_not_awaited()
    mock_kira.assert_not_awaited()
...
async def test_kira_backend_parses_json() -> None:
    with (
        patch("app.kira.sentiment.settings") as mock_settings,
        patch("app.kira.sentiment.call_kira", AsyncMock(return_value='{"sentiment": "negative"}')),
        patch("app.kira.sentiment.kira_is_enabled", AsyncMock(return_value=True)),
    ):
        mock_settings.sentiment_backend = "kira"
        result = await classify_sentiment("Kịch bản dở quá, xem phí tiền vé")

    assert result == "negative"


async def test_auto_falls_back_to_kira_when_phobert_none() -> None:
    with (
        patch("app.kira.sentiment.settings") as mock_settings,
        patch("app.kira.sentiment._classify_phobert", AsyncMock(return_value=None)),
        patch("app.kira.sentiment._classify_kira", AsyncMock(return_value="neutral")) as mock_kira,
    ):
        mock_settings.sentiment_backend = "auto"
        result = await classify_sentiment("Không biết nên khen hay chê phim này luôn")

    assert result == "neutral"
    mock_kira.assert_awaited_once()


async def test_kira_malformed_json_returns_none() -> None:
    with (
        patch("app.kira.sentiment.settings") as mock_settings,
        patch("app.kira.sentiment.call_kira", AsyncMock(return_value="not json at all")),
        patch("app.kira.sentiment.kira_is_enabled", AsyncMock(return_value=True)),
    ):
        mock_settings.sentiment_backend = "kira"
        result = await classify_sentiment("Không biết nên khen hay chê phim này luôn")
...
async def test_kira_rate_limit_returns_none() -> None:
    async def raise_rate_limit(**kwargs):
        raise openai.RateLimitError("rate limited", response=AsyncMock(), body=None)

    with (
        patch("app.kira.sentiment.settings") as mock_settings,
        patch("app.kira.sentiment.call_kira", AsyncMock(side_effect=raise_rate_limit)),
        patch("app.kira.sentiment.kira_is_enabled", AsyncMock(return_value=True)),
    ):
        mock_settings.sentiment_backend = "kira"
